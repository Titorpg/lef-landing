...
(function () {
  ...
  var SECTIONS = [
    ...
    { id: "calendario", label: "Calendario", roles: ["admin", "teacher"] },
  ...
  // Calendario propio de LEF (lef-calendar.js), ya sin Google Calendar. Las
  // clases salen solas de los grupos; el profesor anota cosas de clase para él
  // o para uno de sus grupos; el admin publica para todos, solo profesores,
  // solo estudiantes, un grupo o solo para él. Los estudiantes lo ven en su
  // portal (solo lectura).
  function secCalendario(main) {
    head(main, "Calendario", ME.role === "admin"
      ? "Clases de todos los grupos y lo que publiques para profesores, estudiantes o un grupo. Toca un día para agregar algo."
      : "Tus clases y lo que programes para ti o para tus grupos. Toca un día para agregar algo.");
    if (ME.role === "teacher" && !ME.teacher_id) {
      main.appendChild(h('<div class="pnl-alert err">Tu cuenta no está vinculada a un profesor todavía — pide al admin que la revise en Usuarios.</div>'));
      return;
    }
    var box = h("<div></div>");
